    ...
    @if(!$isEdit)
        ...
        <div>
            ...
            <select name="user_id" required
                class="w-full border border-gray-300 rounded-md px-3 py-2 text-xs focus:ring-2 focus:ring-blue-400 outline-none">
        ...
        <div class="grid grid-cols-2 gap-3 mt-3">
            <div>
                ...
                <input type="date" name="tanggal_pinjam" value="{{ date('Y-m-d') }}" required
                    class="w-full border border-gray-300 rounded-md px-3 py-2 text-xs outline-none">
            </div>
            <div>
                <label class="block text-xs font-medium text-gray-600 mb-1">Jatuh Tempo</label>
                <input type="date" name="tanggal_jatuh_tempo" value="{{ date('Y-m-d', strtotime('+7 days')) }}" required
                    class="w-full border border-gray-300 rounded-md px-3 py-2 text-xs outline-none">
            </div>
        </div>
        ...
        <div id="bukuWrapper" class="space-y-3">
            <div class="buku-item border border-gray-100 rounded-lg p-3 space-y-2 bg-gray-50 relative">
                ...
                <div>
                    ...
                    <select name="buku_id[]" class="bukuSelect w-full border border-gray-300 rounded-md px-3 py-2 text-xs"
                        required></select>
                </div>
                <div>
                    <label class="block text-xs font-medium text-gray-600 mb-1">Jumlah</label>
                    <input type="number" name="jumlah[]" value="1" min="1"
                        class="w-full border border-gray-300 rounded-md px-3 py-2 text-xs">
                </div>
            </div>
        </div>

        <div class="flex justify-between items-center mt-3 pt-2">
            <button type="button" onclick="tambahBuku()"
                class="text-xs text-blue-500 hover:underline flex items-center gap-1 font-semibold">
                + Tambah Buku Lain
            </button>
            <div class="flex gap-2">
                <button type="button" data-modal-hide="crud-modal"
                    class="px-4 py-2 text-xs rounded-md border text-gray-600 hover:bg-gray-50">Batal</button>
                <button type="submit" class="px-4 py-2 text-xs rounded-md bg-blue-500 text-white hover:bg-blue-600">Simpan
                    Peminjaman</button>
            </div>
        </div>
    ...
    @else
        ...
        <div class="space-y-3 text-left">
            ...
            <div>
                ...
                <input type="text" value="{{ $peminjaman->user?->nama ?? $peminjaman->user?->name ?? 'User Terhapus' }}"
                    readonly
            ...
            <div class="grid grid-cols-2 gap-3 items-end bg-blue-50/60 p-3 rounded-xl border border-blue-100">
                ...
                <div>
                    ...
                    <select name="status"
                        class="w-full border border-blue-300 bg-white rounded-md px-3 py-2 text-xs font-bold text-blue-900 focus:ring-2 focus:ring-blue-500 outline-none cursor-pointer shadow-2xs">
                        <option value="menunggu_konfirmasi" @selected($peminjaman->status == 'menunggu_konfirmasi')>Menunggu
                            ACC</option>
                        ...
                        <option value="terlambat" @selected($peminjaman->status == 'terlambat')>Terlambat</option>
                        <option value="ditolak" @selected(in_array($peminjaman->status, ['ditolak', 'dibatalkan']))>
                            Batalkan / Tolak</option>
                    </select>
                </div>
            </div>
                ...
                @foreach($peminjaman->detail as $item)
                        ...
                        <div class="flex items-center justify-between bg-white border border-gray-100 rounded-xl p-2.5 shadow-2xs">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    <p class="text-xs font-bold text-gray-800 line-clamp-1">
                                        {{ $item->buku?->judul ?? 'Buku Terhapus' }}</p>
                            ...
                            <div class="text-right">
                                <span
                                    class="px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider border {{ $itemBadge }}">
                                ...
                                @if($item->tanggal_kembali)
                                    <p class="text-[9px] text-gray-400 mt-1">Kembali:
                                        {{ date('d/m/y', strtotime($item->tanggal_kembali)) }}</p>
                                @endif
                            </div>
                        </div>
                    </div>
        ...
        <div class="flex justify-end gap-2 pt-3 border-t border-gray-100 order-2">
            <button type="button" data-modal-hide="detail-pinjam-{{ $peminjaman->id_peminjaman }}"
                class="px-4 py-2 text-xs rounded-md border text-gray-600 hover:bg-gray-50">Tutup</button>
            <button type="submit"
                class="px-4 py-2 text-xs rounded-md bg-blue-600 text-white hover:bg-blue-700 font-semibold shadow-sm">Simpan
                Perubahan Status</button>
        </div>
    </form>


            <form action="{{ route('admin.peminjaman.destroy', $peminjaman->id_peminjaman) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus data peminjaman ini?')" class="order-1">
    @csrf
    @method('DELETE')
    <button type="submit" class="px-4 py-2 text-xs rounded-md bg-red-600 text-white hover:bg-red-800 font-bold text-xs">
        Hapus Data
    </button>
</form>
        </div>
