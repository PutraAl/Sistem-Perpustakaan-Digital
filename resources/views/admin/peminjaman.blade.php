...
@section('content')
    <div class="mb-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
            <h1
                class="text-2xl font-bold bg-gradient-to-r from-blue-600 via-cyan-500 to-indigo-500 bg-clip-text text-transparent">
    ...
    <div x-data="{ open: true }" class="mb-6">
        <div
            class="bg-white/70 backdrop-blur-lg border border-white/30 rounded-2xl p-5 shadow-xl shadow-blue-100/50 transition-all duration-300">
            <div class="flex items-center justify-between mb-4">
                ...
                <button @click="open = !open" class="text-gray-400 hover:text-blue-500 transition-transform"
            ...
            <div x-show="open" x-collapse>
                <form method="GET" action="{{ route('admin.peminjaman') }}" class="space-y-4">
                    <div class="flex flex-col md:flex-row gap-3">
                        ...
                        <div class="relative">
                            <select name="status" class="appearance-none w-full md:w-52 rounded-xl border border-gray-200 bg-gray-50 px-4 py-3 pr-10 focus:ring-2 focus:ring-blue-400 outline-none transition-all text-sm
                                    @if(request('status') == 'menunggu_konfirmasi') border-amber-300 bg-amber-50 text-amber-700
                                    @elseif(request('status') == 'dipinjam') border-blue-300 bg-blue-50 text-blue-700
            ...
            @php
                ...
                if (request('status'))
                    $filters[] = 'Status: ' . ucwords(str_replace('_', ' ', request('status')));
                if (request('start_date'))
                    $filters[] = 'Dari: ' . \Carbon\Carbon::parse(request('start_date'))->format('d M Y');
                if (request('end_date'))
                    $filters[] = 'Sampai: ' . \Carbon\Carbon::parse(request('end_date'))->format('d M Y');
            @endphp
            @if(count($filters))
                <div class="flex flex-wrap gap-2 mt-4 pt-4 border-t border-gray-100">
                    @foreach($filters as $filter)
                        <span
                            ...
                            {{ $filter }}
                            <button onclick="this.closest('span').remove(); document.querySelector('form').submit();"
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="bg-gradient-to-r from-slate-50 to-blue-50">
                        ...
                        <th class="text-left text-xs font-semibold text-gray-400 pb-2 pr-4 py-3 hidden sm:table-cell">
                            Tanggal Pinjam</th>
                        <th class="text-left text-xs font-semibold text-gray-400 pb-2 pr-4 py-3 hidden sm:table-cell">Jatuh
                            Tempo</th>
                        <th class="text-left text-xs font-semibold text-gray-400 pb-2 py-3">Status</th>
                        <th class="text-left text-xs font-semibold text-gray-400 pb-2 py-3">Denda</th>
                ...
                <tbody class="divide-y divide-gray-50">
                    @forelse($data as $peminjaman)
                        <tr class="hover:bg-blue-50/50 transition duration-200">
                            ...
                            {{-- PERBAIKAN: Kolom Judul Buku bisa menampilkan "+ sisa buku" --}}
                            <td class="py-3 pr-4 text-gray-800 text-xs px-4">
                                @php
                                    $bukuPertama = $peminjaman->detail->first()?->buku?->judul ?? 'Buku Dihapus';
                                    $sisaBuku = $peminjaman->detail->count() - 1;
                                @endphp
                                <span class="font-medium">{{ $bukuPertama }}</span>
                                @if($sisaBuku > 0)
                                    <span
                                        class="ml-1 text-[10px] text-blue-600 font-semibold bg-blue-100 px-1.5 py-0.5 rounded shadow-sm">
                                        +{{ $sisaBuku }} buku
                                    </span>
                                @endif
                            </td>

                            {{-- Memastikan relasi user aman dari error null --}}
                            <td class="py-3 pr-4 text-gray-600 text-xs font-medium">
                                {{ $peminjaman->user?->name ?? $peminjaman->user?->nama ?? 'User Terhapus' }}
                            </td>
                            ...
                            {{-- PERBAIKAN: Badge Status lengkap untuk 5 skenario --}}
                            <td class="py-3 pr-4">
                                @if($peminjaman->status == 'menunggu_konfirmasi')
                                    <span
                                        class="px-2.5 py-1 rounded-full text-[10px] font-semibold bg-amber-100 text-amber-700">Menunggu
                                        ACC</span>
                                @elseif($peminjaman->status == 'dipinjam')
                                    <span
                                        class="px-2.5 py-1 rounded-full text-[10px] font-semibold bg-blue-100 text-blue-700">Dipinjam</span>
                                @elseif($peminjaman->status == 'terlambat')
                                    <span
                                        class="px-2.5 py-1 rounded-full text-[10px] font-semibold bg-red-100 text-red-700">Terlambat</span>
                                @elseif($peminjaman->status == 'dikembalikan')
                                    <span
                                        class="px-2.5 py-1 rounded-full text-[10px] font-semibold bg-green-100 text-green-700">Dikembalikan</span>
                                @elseif(in_array($peminjaman->status, ['ditolak', 'dibatalkan']))
                                    <span
                                        class="px-2.5 py-1 rounded-full text-[10px] font-semibold bg-slate-100 text-slate-600">Ditolak</span>
                                @endif
                            </td>
                            ...
                            <td class="py-3">
                                <button data-modal-target="detail-pinjam-{{ $peminjaman->id_peminjaman }}"
                                    data-modal-toggle="detail-pinjam-{{ $peminjaman->id_peminjaman }}"
                                    class="px-3 py-1 rounded-full text-[10px] font-bold tracking-wider bg-blue-500 text-white hover:cursor-pointer hover:bg-blue-600 hover:shadow-md transition-all">
                                    Detail
                                </button>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="6" class="text-center py-8 text-gray-400 font-medium">Belum ada data peminjaman di
                                sistem.</td>
                        </tr>
                    @endforelse
                </tbody>
    ...
    @foreach($data as $peminjaman)
        <x-modal id="detail-pinjam-{{ $peminjaman->id_peminjaman }}"
            title="Detail Peminjaman #{{ $peminjaman->id_peminjaman }}">
            <x-forms.form-peminjaman :peminjaman="$peminjaman" :users="$users" :bukus="$bukus" />
        </x-modal>
    @endforeach
